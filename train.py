...
def get_imagefolder_dataset(dataset_name, split, transform):
    root = os.path.join('/home/dell/桌面/Liyuelong/datasets', dataset_name, split)
...
def run_training():
    ...
    elif args.dataset.lower() in ["raf-db", "affectnet_folder", "fer-2013", "ferplus", "before_raf-db"]:
        ...
        with torch.no_grad():
            ...
            if val_acc > best_acc:
                torch.save({'iter': i,
                           ...
                           os.path.join('./checkpoint', "epoch" + str(i) + "_acc" + str(val_acc) + args.dataset + ".pth"))
                print('Model saved.')
            if val_acc > best_acc:
                best_acc = val_acc
    ...
    df.to_excel(f'./logs/train_log_{args.dataset}.xlsx', index=False)
