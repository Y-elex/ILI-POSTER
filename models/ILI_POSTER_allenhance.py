...
class LowLightEnhancer(nn.Module):
    ...
    def is_low_light_batch(self, img_batch):
        ...
        for i in range(B):
            ...
            is_low = True  # 直接增强所有图像，跳过判别逻辑（可选）
...
class pyramid_trans_expr(nn.Module):
    def __init__(self, img_size=224, num_classes=7, type="large", negative_emotions=None, neg_weight=1.0):
        super().__init__()
        depth = 8
        ...
        if negative_emotions is None:
            # 默认 RAF-DB 负面情绪索引
            negative_emotions = [2, 4, 5, 6]  # fear, disgust, sadness, anger
